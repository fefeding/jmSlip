...
var _default = function (win, doc) {
  ...
  function slip(el, mode, option) {
    ...
    this.offsetY = 0;
    this.option = option; //css(doc.body, 'margin', 0);
  ...
  slip.prototype.bindEvent = function (b) {
    ...
    function onTouchStart(evt) {
      ...
      if (self.option && self.option.onTouchStart && typeof self.option.onTouchStart == 'function') {
        var stop = self.option.onTouchStart.call(self.slipObj, evt); //如果回调返回false , 则中止当前滑动
    ...
    function onTouchMove(evt) {
      ...
      if (self.touched) {
        var offsetPos = []; //先把上回的点赋值给prePosition
    ...
    function onTouchEnd(evt) {
      evt = evt || win.event; //console.log(evt);
      ...
      if (self.touched) {
        ...
        self.touched = false;
        self.auto(); //evt.stopPropagation && evt.stopPropagation();
        //evt.preventDefault && evt.preventDefault();//阻止默认响应
      }
    } //滚轮事件
    ...
    function onMousewheel(e) {
      ...
      wheelInterval = setTimeout(function () {
        ...
        if (wheelOff > 0) self.slipObj.previous && self.slipObj.previous();else if (wheelOff < 0) self.slipObj.next && self.slipObj.next();
        wheelOff = 0;
      }, 500);
    }
    ...
    if (this.option.mousewheel) {
      if (b === false) unbind(doc, 'mousewheel', onMousewheel);else {
        bind(doc, 'mousewheel', onMousewheel);
      }
    }
  };
  ...
  slip.prototype.auto = function () {
    ...
    if (this.option.changeTime && this.slipObj.go) {
      var intervalHandler = function intervalHandler() {
        ...
        var page = self.slipObj.page + dir; //当前有多少页

        var pageCount = self.containerInner.children.length;
        if (self.slipObj.children && self.slipObj.children.length) pageCount = self.slipObj.children.length; //如果设置循环处理，则不需要变动方向
  ...
  function pageSlip(instance) {
    ...
    if (instance && instance.containerInner) {
      ...
      if (this.instance.containerInner.children[this.page]) this.instance.containerInner.children[this.page].style.zIndex = this.option.zIndex;
    }
  } //初始化子页
  ...
  pageSlip.prototype.initChildren = function () {
    ...
    if (this.instance && this.instance.containerInner) {
      ...
      for (var i = 0; i < this.instance.containerInner.children.length; i++) {
        var ch = this.instance.containerInner.children[i]; //如果在属性中没有设置page属性，则表示为后续补齐的，设置其page
  ...
  pageSlip.prototype.reset = function () {
    if (this.instance.option.direction == 'x') {
      this.pageWidth = this.instance.option.width || this.instance.container.offsetWidth; //如果是默认的翻页方式，则内框的宽度为子元素总宽度和
      //其它动画效果为容器宽高，使用绝对定位
      //css(this.instance.containerInner, 'width', this.pageWidth + 'px');
      //his.instance.setStyle('width', this.pageWidth + 'px');
    } else {
      this.pageHeight = this.instance.option.height || this.instance.container.offsetHeight; //css(this.instance.containerInner, 'height', this.pageHeight + 'px');
      //this.instance.setStyle('height', this.pageHeight + 'px');
    }

    var len = this.instance.containerInner.children.length; //如果设置了循环，且只有二个元素，则复制为4个
  ...
  pageSlip.prototype.move = function (offx, offy) {
    ...
    var nextpage = this.children[parseInt(this.page || 0, 10) + 1]; //当前页没加入则直接加入

    if (curpage && !curpage.parentNode && !this.option.repeat) this.instance.containerInner.appendChild(curpage); //如果设置了循环，则最后一面后再回到第一页

    if (this.option.loop) {
      //如果没有上一页，则循环到最后一页
      if (!prepage) prepage = this.children[this.children.length - 1]; //如果没有下一页，则循 环到第一页
  ...
  pageSlip.prototype.end = function (offx, offy, xdirection, ydirection, evt) {
    if (this.instance.option.direction == 'x') {
      if (offx > minOffset && xdirection == 'right') this.previous();else if (offx < -minOffset && xdirection == 'left') this.next();else this.go(this.page);
    } else {
      if (offy > minOffset && ydirection == 'down') this.previous();else if (offy < -minOffset && ydirection == 'up') this.next();else this.go(this.page);
    }
  };
  ...
  pageSlip.prototype.go = function (page, actionType) {
    ...
    if (this.option && this.option.onPageStart && typeof this.option.onPageStart == 'function') {
      var stop = this.option.onPageStart.call(this, page, oldpage, actionType); //如果回调返回false , 则中止
    ...
    this.page = this.instance.page = page; //只保留 三页
    ...
    if (!this.option.repeat) {
      ...
      if (oldpage > page) {
        if (!this.option.repeat) {
          ...
          if (lastpage && curpage != lastpage && lastpage.parentNode) this.instance.containerInner.removeChild(lastpage); //如果是从最后一页到第一页，则下一页为第二页,加入后面
        ...
        var firstpage = this.children[page - 1]; //如果有设置循环翻页，则第一页后跳到最后一页
        ...
        if (firstpage) {
          ...
          if (!this.option.repeat && !firstpage.parentNode) {
            ...
            if (curpage.parentNode) this.instance.containerInner.insertBefore(firstpage, curpage);else this.instance.containerInner.appendChild(firstpage);
          }
        }
        ...
        var lastpage = this.children[page + 1]; //如果有设置循环翻页，则最后一页后面跟着的是第一页
        ...
        if (lastpage) {
          ...
          for (var i = chlen - 1; i >= 0; i--) {
            ...
            if (i < page - 1 || i > page + 1) {
              if (page == 0 && this.option.loop && i == chlen - 1) continue;else if (page == chlen - 1 && this.option.loop && i == 0) continue;
    ...
    this.move(0, 0); //翻页后先调用自定义回调，以备做一些其它自定义处理
  ...
  pageSlip.prototype.pageEnd = function (srcPage, desPage) {
    ...
    toggleClass(page.querySelectorAll('.jmslip-ani'), 'jmslip-ani-normal', true); //先去除动画效果，让元素到动画前位置，再加上动画基类，为了过去的页面改变不出现动画效果。
  ...
  scrollSlip.prototype.reset = function () {
    var len = this.instance.containerInner.children.length; //横向滑动的话，容器 为每个子元素宽度和
  ...
  scrollSlip.prototype.end = function (offx, offy, xdirection, ydirection, evt) {
    if (this.instance.option.direction == 'x') {
      if (this.offsetX > 0) this.reset();else {
      ...
      if (this.offsetY > 0) this.reset();else {
  ...
  itemSlip.prototype.end = function (offx, offy, xdirection, ydirection, evt) {
    ...
    if (this.instance.option.direction == 'x') {
      if (offx > minOffset || offx < -minOffset) {
        var index = this.getCenterIndex(offx, offy); //如果有自定义选中的项，则采用自定义的结果

        if (this.instance.option.selectHandler) {
          var newindex = this.instance.option.selectHandler(this.page, index);
          if (typeof newindex == 'number' && newindex != index) index = newindex;else if (newindex === false) {
  ...
  itemSlip.prototype.go = function (page) {
    ...
    var oldpage = this.page;
    if (page < 0) page = 0;else if (page > len - 1) page = len - 1; //如果页面没有改变。则不执行后面的
    //if(page == oldpage) return;
    //翻页前先调用自定义回调，如果返回false则中止

    if (this.option && this.option.onPageStart && typeof this.option.onPageStart == 'function') {
      var stop = this.option.onPageStart.call(this, page); //如果回调返回false , 则中止
    ...
    this.move(offx, offy);
    this.page = this.instance.page = page; //翻页后先调用自定义回调，以备做一些其它自定义处理
  ...
  itemSlip.prototype.getCenterIndex = function (offx, offy) {
    if (this.instance.option.direction == 'x') {
      ...
      var leftwidth = 0;
      var lastoffx = 0;
      var mleft = this.instance.container.offsetWidth / 2 - this.offsetX; //找到离中间最近的项
      ...
      var topheight = 0;
      var lastoffy = 0;
      var mtop = this.instance.container.offsetHeight / 2 - this.offsetY; //找到离中间最近的项
  ...
  function dragSlip(instance) {
    this.instance = instance; //把容器重置为document

    instance.container = doc;
    instance.containerInner = instance.target;
    this.option = instance.option;
    this.offsetY = 0;
    this.offsetX = 0; //拖放时，不响应默认效果
  ...
  dragSlip.prototype.offset = function (offx, offy, evt) {
    offx += this.offsetX;
    offy += this.offsetY; //evt && evt.preventDefault && evt.preventDefault();//阻止默认响应
  ...
  scrollItemSlip.prototype.reset = function (target) {
    ...
    setTimeout(function () {
      ...
      for (var i = 0; i < len; i++) {
        var target = self.instance.container.children[i]; //如果有指定CSS选择器
  ...
  scrollItemSlip.prototype.setTransition = function (b, el) {
    ...
    for (var i = 0; i < len; i++) {
      var target = this.instance.container.children[i]; //如果有指定CSS选择器
  ...
  scrollItemSlip.prototype.offset = function (offx, offy, evt) {
    if (!evt) return;
    var target = evt.target || evt.srcElement; //如果有指定CSS选择器
    ...
    if (this.instance.option.direction == 'x') {
      ...
      if (Math.abs(offx) > Math.abs(offy)) {
        ...
        offx += tx;
        var maxoff = this.option.maxOffset || target.offsetWidth; //如果有指定滑向，则不能反向滑

        if (this.option.orientation == 'left' && (offx > 0 || offx < -maxoff)) return;else if (this.option.orientation == 'right' && (offx < 0 || offx > maxoff)) return;
        evt && evt.preventDefault && evt.preventDefault(); //阻止默认响应
      } else offx = false;
      ...
      if (Math.abs(offy) > Math.abs(offx)) {
        ...
        offy += ty;
        var maxoff = this.option.maxOffset || target.offsetHeight; //如果有指定滑向，则不能反向滑

        if (this.option.orientation == 'top' && (offy > 0 || offy < -maxoff)) return;else if (this.option.orientation == 'bottom' && (offy < 0 || offy > maxoff)) return;
        evt && evt.preventDefault && evt.preventDefault(); //阻止默认响应
      } else offy = false;
  ...
  scrollItemSlip.prototype.end = function (offx, offy, xdirection, ydirection, evt) {
    ...
    if (this.instance.option.direction == 'x') {
      ...
      if (!tx) return; //如果只能左向滑，则复位

      if (tx > 0 && this.option.orientation == 'left' || tx < 0 && this.option.orientation == 'right') this.reset(target);else {
        var maxoff = this.option.maxOffset || target.offsetWidth; //左向滑，如果超过了一半最大滑动，则滑满
      ...
      if (!ty) return;
      if (ty > 0 && this.option.orientation == 'top' || ty < 0 && this.option.orientation == 'bottom') this.reset(target);else {
        var maxoff = this.option.maxOffset || target.offsetHeight; //上向滑，如果超过了一半最大滑动，则滑满
    ...
    this.last_target = target; //滚动事件回调
  ...
  scaleSlip.prototype.offset = function (offx, offy, evt) {
    ...
    if (evt.offsetPos && evt.offsetPos.length == 2) {
      ...
      var preDis = Math.sqrt(preOffX * preOffX + preOffY * preOffY); //当次滑动二指的距离
      ...
      if (!cW || !cH) return; //计算放大比例

      var offxper = disx / cW;
      var offyper = disy / cH; //往外拉，则表示放大
      ...
      this.scale(per);
        ...
        if (offx === false) offx = this.offsetX; //如果被缩放，移动的距离要处理缩放比例，不然会漂
        else offx /= this.scaleX;
        if (offy === false) offy = this.offsetY; //如果被缩放，移动的距离要处理缩放比例，不然会漂
  ...
  scaleSlip.prototype.set = function (v, offx, offy) {
    if (!v) v = this.scaleX; //如果指定了最大缩放比例

    if (this.option.maxScale && v > this.option.maxScale) v = this.option.maxScale; //如果指定了最小缩放比例
  ...
  scaleSlip.prototype.setTransition = function (b) {}; //设置对象样式
  ...
  function attr(el, name, value) {
    ...
    if (typeof value == 'undefined') {
      ...
      if (att) att.nodeValue = value;else {
  ...
  function elParent(el, selector) {
    ...
    if (sel && sel == el) return el; //如果没有找到，，则继续从父元素找，直接找完为止
    else {
